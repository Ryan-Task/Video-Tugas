<?php 
    ...
    if (isset($_GET['total'])) {
        ...
        $sql = "SELECT * FROM tborder WHERE idorder = $idorder";
    ...
    function idorder(){
        global $db;
        $sql = "SELECT idorder FROM tborder ORDER BY idorder DESC";
    ...
    function inserOrder($idorder,$idpelanggan,$tgl,$total){
        ...
        $sql = "INSERT INTO tborder VALUES ($idorder,$idpelanggan,'$tgl',$total,0,0,0 )";
        ...
        foreach ($_SESSION as $key => $value) {
            if ($key<>'pelanggan' && $key<>'idpelanggan') {
                ...
                foreach ($row as $r) {
                    $idmenu =$r['idmenu'];
                    $harga =$r['harga'];
                    $sql = "INSERT INTO tborderdetail VALUES ('',$idorder,$idmenu,$value,$harga)";
                    $db->runSQL($sql);
                }
